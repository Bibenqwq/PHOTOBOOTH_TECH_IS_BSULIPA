...
export default function App() {
    ...
    const [bsuLogoImg, setBsuLogoImg] = useState(null);

    const currentLayoutObj = LAYOUTS.find(l => l.id === selectedLayout) || LAYOUTS[2];
    const photoCount = currentLayoutObj.count;

    // Preload brand and university logos
    useEffect(() => {
        const img = new Image();
        img.src = '/logo.png';
        img.onload = () => setLogoImg(img);

        const bsuImg = new Image();
        bsuImg.src = '/Batangas_State_Logo.png';
        bsuImg.onload = () => setBsuLogoImg(bsuImg);
    }, []);
